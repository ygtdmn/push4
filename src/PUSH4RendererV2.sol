...
/**
 * @title PUSH4RendererV2
 * @author Yigit Duman
 * @dev Changelog: Added automatic visual refreshing at block intervals using MURI Protocol-supported RPC URLs.
 */
contract PUSH4RendererV2 is IPUSH4RendererV2, Ownable {
    ...
    /// @param _blockInterval The block interval for animation
    ...
    constructor(
        ...
        Ownable(_owner)
    {
        if (_blockInterval == 0) revert InvalidBlockInterval();
        width = _width;
        height = _height;
    ...
    function getMetadata() public view returns (string memory) {
        if (push4Core.mode() == IPUSH4Core.Mode.Carved) {
            return string(abi.encodePacked("{", _metadata, ', "image": "', getSvgDataUri(), '"}'));
        }

        return string(
            abi.encodePacked(
                "{", _metadata, ', "image": "', getSvgDataUri(), '", "animation_url": "', getAnimationUrl(), '"}'
            )
        );
    }
    ...
    function setPixelSize(uint256 _pixelSize) external onlyOwner {
        pixelSize = _pixelSize;
    }

    /// @notice Sets the PUSH4Core contract
    /// @param _push4Core The new PUSH4Core contract
    function setPush4Core(IPUSH4Core _push4Core) external onlyOwner {
        push4Core = _push4Core;
    }

    /// @notice Sets the metadata
    /// @param __metadata The new metadata
    function setMetadata(string memory __metadata) external onlyOwner {
        _metadata = __metadata;
    }
    ...
    function setBlockInterval(uint256 _blockInterval) external onlyOwner {
        if (_blockInterval == 0) revert InvalidBlockInterval();
        uint256 oldValue = blockInterval;
        blockInterval = _blockInterval;
        emit BlockIntervalUpdated(oldValue, _blockInterval);
    }

    function getAnimationUrl() public view returns (string memory) {
        if (address(muriProtocol) == address(0)) {
            return "";
        }

        string memory html = muriProtocol.renderRawHTML(address(push4Core), TOKEN_ID);

        html = LibString.replace(html, "{{BLOCK_INTERVAL}}", LibString.toString(blockInterval));
        html = LibString.replace(html, "{{CORE_ADDRESS}}", LibString.toHexStringChecksummed(address(push4Core)));
        html = LibString.replace(html, "{{TOKEN_ID}}", LibString.toString(TOKEN_ID));
