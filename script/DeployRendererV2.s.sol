...
contract DeployRendererV2 is BaseScript {
    ...
    function run()
        ...
        broadcast
        returns (
            PUSH4ProxyFactory factory,
            PUSH4RendererV2 rendererV2,
            PUSH4OrchestratorProxy orchestrator,
            PUSH4OriginalProxy originalProxy,
            PUSH4MURIOperator muriOperator
        )
    {
        // Deploy ProxyFactory
        ...
        orchestrator = new PUSH4OrchestratorProxy(broadcaster, rendererV2, IERC721(address(push4Core)));

        // Deploy original proxy via factory
        bytes memory originalProxyBytecode = type(PUSH4OriginalProxy).creationCode;
        originalProxy = PUSH4OriginalProxy(address(factory.deploy(originalProxyBytecode)));
        orchestrator.registerProxy(originalProxy);
        IMURIProtocol muriProtocol = IMURIProtocol(MURI_PROTOCOL);
        muriOperator = new PUSH4MURIOperator(orchestrator, rendererV2, muriProtocol, broadcaster);
        ...
        string[] memory artistUris = new string[](4);
        artistUris[0] = "https://eth.drpc.org";
        artistUris[1] = "https://1rpc.io/eth";
        artistUris[2] = "https://ethereum-rpc.publicnode.com";
        artistUris[3] = "https://eth.llamarpc.com";
